...
int main(int argc, char** argv) 
{
  ...
  //  }
    ...
    x_est = current_state.pose.pose.position.x;
    x_est_old = x_est;
    y_est =current_state.pose.pose.position.y;
    y_est_old = y_est;
    yaw_est = current_state.pose.pose.orientation.z;
    for(int initializationIndex = 0; initializationIndex<5; initializationIndex++){
        ros::spinOnce();
        x_est = current_state.pose.pose.position.x;
        x_est_old = x_est;
        y_est =current_state.pose.pose.position.y;
