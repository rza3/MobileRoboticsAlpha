...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    nav_msgs::Odometry SPEED=*vec_of_states.end();
    double speed_des=SPEED.twist.twist.linear.x;
    
    
    double t_ramp =  speed_des/accel_max_;
    double n_pts = t_ramp/dt_;


   // double omega_des=  vec_of_states.angular.z  ;
    //double accel=-sgn(omega_des)*accel_max_;
    //double t_ramp_spin = omega_des/accel;
    //double n_pts_spin = t_ramp/dt_;
