...
void amclXYCallback(const geometry_msgs::PoseWithCovarianceStamped& amcl_pose){
    x_amcl=amcl_pose.pose.x;
    y_amcl=amcl_pose.pose.x;
    g_amcl_pose=amcl_pose;
    g_amcl_good= true;
}
//amcl orientation
void amclOriCallback(const geometry_msgs::PoseWithCovarianceStamped& amcl_pose) {
   geometry_msgs::Quaternion  state_quat = amcl.orientation;
...
int main(int argc, char** argv) 
{
    ...
    ros::Subscriber amcl_subscriber = nh.subscribe("gazebo_mobot_noisy_pose", 1, amclXYCallback); 
    ...
    ros::Subscriber true_state_subscriber = nh.subscribe("gazebo_mobot_pose",1,amclOriCallback);
